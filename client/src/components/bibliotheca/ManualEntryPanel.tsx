...
import { ManualDraft, ReadingStatus, GeneratedCoverDef, CoverStyle } from "@/types/bibliotheca";
...
const COVER_STYLES: { id: CoverStyle; label: string; color: string }[] = [
  { id: "minimal", label: "Minimal", color: "#1c2024" },
  { id: "modernist", label: "Moderno", color: "#14171a" },
  { id: "geometric", label: "Geométrico", color: "#192024" },
  { id: "banded", label: "Faixas", color: "#1a2332" },
  { id: "typographic", label: "Tipografia", color: "#2b1b17" },
];
...
export function ManualEntryPanel({ initialIsbn = "", onConfirm, onBack }: ManualEntryPanelProps) {
  ...
  const dupeStatus = title.trim() ? checkDuplicate(normIsbn, title, author || undefined) : "none";
  const showDupeWarning = !forceAdd && dupeStatus !== "none";

  const cover: GeneratedCoverDef = { ...DEFAULT_COVER, style: coverStyle };
  ...
  const sectionClass = "text-[10px] font-cinzel uppercase tracking-widest text-[var(--ancient-gold)]/50 mb-2";
  ...
  const canSubmit = title.trim().length > 0 && !(showDupeWarning && dupeStatus === "exact-edition");
      ...
      <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
        ...
        <div className="flex gap-4 items-start">
          ...
          <div className="flex-1">
            <p className={sectionClass}>Estilo de capa</p>
            <div className="flex flex-wrap gap-1.5">
              {COVER_STYLES.map((s) => (
                <button
                  ...
                  title={s.label}
                  className={`w-7 h-9 rounded-sm border transition-all ${
                    coverStyle === s.id
                      ? "border-[var(--byzantine-gold)] ring-1 ring-[var(--byzantine-gold)]/40"
                      : "border-white/10 hover:border-[var(--ancient-gold-warm)]/40"
                  }`}
        ...
        {/* Work section */}
        <div>
          <p className={sectionClass}>Livro</p>
          <div className="space-y-2">
            <div>
              <label className={labelClass}>Título *</label>
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Título da obra" className={fieldClass} />
            </div>
            {!author.trim() && title.trim() && (
              <p className="text-[11px] font-inter text-amber-400/80 flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" /> Autor não preenchido. Será registrado como desconhecido.
              </p>
            )}
            <div>
              <label className={labelClass}>Autor</label>
              <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Nome do autor" className={fieldClass} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <input type="number" value={origYear} onChange={(e) => setOrigYear(e.target.value)} placeholder="1869" className={fieldClass} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className={`flex items-start gap-2.5 p-3 rounded-lg border text-xs font-inter ${
            ...
            <div className="flex-1">
              ...
              {dupeStatus === "same-work" && (
                <button type="button" onClick={() => setForceAdd(true)}
        ...
        {/* Edition section */}
        <div>
          <p className={sectionClass}>Sua edição</p>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className={labelClass}>ISBN</label>
                <input type="text" value={isbn} onChange={(e) => setIsbn(e.target.value)} placeholder="978-…" className={fieldClass} />
              </div>
              <div>
            ...
            <div>
              ...
              <input type="text" value={format} onChange={(e) => setFormat(e.target.value)} placeholder="Capa dura, Bolso, Edição bilíngue…" className={fieldClass} />
            </div>
          </div>
        </div>
